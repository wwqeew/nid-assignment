...
def make_candidate_configs():
    # small search around the best experiment 19 setup
    return [
        {
            "name": "cw_1_r2l18_u2r60",
            "hidden_1": 256,
            "hidden_2": 128,
            "hidden_3": 64,
            "dropout": 0.30,
        ...
        {
            "name": "cw_6_normal065_r2l20_u2r60",
            "hidden_1": 256,
            "hidden_2": 128,
            "hidden_3": 64,
            "dropout": 0.30,
        ...
        {
            "name": "cw_7_normal075_r2l20_u2r60",
            "hidden_1": 256,
            "hidden_2": 128,
            "hidden_3": 64,
            "dropout": 0.30,
...
class MacroF1EarlyStopping(tf.keras.callbacks.Callback):
    def __init__(self, X_valid, y_valid, patience):
        ...
        # monitor macro f1 directly because this is the competition metric
...
def train_with_macro_f1_stopping(X_fit, y_fit, X_valid, y_valid, config, seed):
    # used only for validation tuning
    model = make_model(
...
def train_fixed_epoch_model(X_processed, y_encoded, config, seed, epochs):
    # final model uses full training data and fixed epoch count from validation tuning
    model = make_model(
...
def tune_configs_on_validation(X_train, y_train_encoded, seed):
    # tuning uses only kddtrain+, not kddtest+
    ...
    print("\n===== Keras fine tuning around best weights =====")
    print(f"Seed used for all configs: {seed}")
    ...
    for config in candidate_configs:
        ...
        model, callback_best_f1, best_epoch = train_with_macro_f1_stopping(
            X_fit,
            y_fit,
            X_valid,
            y_valid,
...
def train_final_and_evaluate(best_result, X_train, y_train_encoded, X_test, y_test, label_encoder):
    ...
    return y_test_pred, test_macro_f1, elapsed_time


def run_cv(best_result, X_train, y_train_encoded):
    # cv for the selected best setup
    print("\nRunning 3-fold cross-validation for selected setup...")
    ...
    for fold_id, (train_index, valid_index) in enumerate(cv.split(X_train, y_train_encoded), start=1):
        ...
        model, inner_best_f1, inner_best_epoch = train_with_macro_f1_stopping(
            X_fit,
            y_fit,
            X_inner_valid,
            y_inner_valid,
        ...
        best_epochs.append(inner_best_epoch)

        print(f"Fold {fold_id} inner validation macro F1: {inner_best_f1:.4f}")
        print(f"Fold {fold_id} best epoch: {inner_best_epoch}")
...
def main():
    ...
    # seed 100 was best in experiment 19
    seed = 100
    ...
    y_test_pred, test_macro_f1, final_eval_time = train_final_and_evaluate(
        best_result,
        X_train,
        y_train_encoded,
        X_test,
    ...
    save_confusion_matrix(
        y_test,
        y_test_pred,
        "outputs/confusion_matrices/20_keras_finetune_best_weights.png"
    )

    total_elapsed_time = time.perf_counter() - total_start_time

    print("\n===== Experiment Summary =====")
    print("Experiment: 20_keras_finetune_best_weights")
    print(f"Best configuration: {best_result['config']['name']}")
    print(f"Best seed: {best_result['seed']}")
    print(f"Best class weights: {best_result['config']['class_weights']}")
    print(f"Best validation macro F1 during tuning: {best_result['valid_macro_f1']:.4f}")
    print(f"Best epoch from validation tuning: {best_result['best_epoch']}")
